...
) const
{
    ...
    return calcWeights(weights, mesh, stencil, target, dir);
}
...
{
    const int nDims = mesh.nSolutionD();
    return calcWeights(weights, mesh.cellCentres(), stencil, target, dir, nDims);
}
...
) const
{
    ...
    return calcWeights(weights, points, stencil, target, dir, nDims);
}
...
(
    scalarList& weights, const pointField& points,
    const point& target, const int nDims
) const
{
    labelList stencil(points.size());
    forAll(stencil, ip){ stencil[ip] = ip; }
    return calcWeights(weights, points, stencil, target, nDims);
}
...
) const
{
    ...
    const scalar centralWeight = 1; //000;
...
//            << " stencilSize = " << N << " reducing order" << endl;
    ...
    )
    {
        ...
        forAll(pts, ip)
        {
            ...
            if (order >= oTHREE)   B[ip][is++] = wts[ip]*pow(px,3);
