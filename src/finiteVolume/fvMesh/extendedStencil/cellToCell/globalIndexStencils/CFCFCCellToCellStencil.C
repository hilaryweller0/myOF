...
#include "CFCFCCellToCellStencil.H"
#include "syncTools.H"
...
void Foam::CFCFCCellToCellStencil::calcFaceBoundaryData
(
    labelList& neiGlobal
) const
{
...
void Foam::CFCFCCellToCellStencil::calcCellStencil(labelListList& globalCellCells)
 const
{
    const label nBnd = mesh().nFaces()-mesh().nInternalFaces();
    const labelList& own = mesh().faceOwner();
    ...
    // Determine cellCellCells in global numbering
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Foam::CFCFCCellToCellStencil::CFCFCCellToCellStencil(const polyMesh& mesh)
:
