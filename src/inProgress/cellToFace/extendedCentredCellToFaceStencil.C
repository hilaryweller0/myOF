...
void Foam::extendedCentredCellToFaceStencil::compact()
{
    ...
    forAll(elements_, faceI)
    {
        // Mark untransformed elements
        {
            const labelList& elems = elements_[faceI];

            forAll(elems, i)
            {
                isInStencil[elems[i]] = true;
            }
        }

        // Mark transformed elements
        {
            const labelList& elems = transformedElements_[faceI];
