...
//        new polyPatch
        ...
        (
            ...
            newMesh.nInternalFaces(), 0, newMesh.boundaryMesh(), "empty"
        )
    );
    newPatches.append
    (
...
//        new polyPatch
        (
            ...
            newMesh.nInternalFaces() + newPatch.size(), 1,
            newMesh.boundaryMesh(), "empty"
        )
    );
    ...
    //    const scalar r2 = radialExtrude.Router();
        const scalar r1 = mag
        (
            radialExtrude(newPatch.points()[0], newPatch.pointNormals()[0], 0)
        );
        const label kOut = radialExtrude.nLayers();
        const scalar r2 = mag
        (
            radialExtrude(newPatch.points()[0], newPatch.pointNormals()[0],kOut)
        );
