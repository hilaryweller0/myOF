...
Foam::label Foam::VoronoiSphereMeshing::addPoints()
{
    ...
    do
    {
        ...
        // Loop through all the facets on the sphere to check the resolution
        for
        (
        ...
        {   //Info << fit << endl;
            // Find the three vertices on this facet, if it is a facet on the sphere's surface
            Vertex_handle vl = fit->first->vertex(fit->second);

            if (vl == vCentral())
            {
                Vertex_handle vi = fit->first->vertex(((fit->second)+1)%4);
                Vertex_handle vj = fit->first->vertex(((fit->second)+2)%4);                
                Vertex_handle vk = fit->first->vertex(((fit->second)+3)%4);
                
                
                //convert into FOAM points
                Foam::point pi = topoint(vi->point());
                Foam::point pj = topoint(vj->point());
                Foam::point pk = topoint(vk->point());

                
                //dx2 for each edge
                scalar dx2ij = magSqr(pi - pj);
                scalar dx2ik = magSqr(pi - pk);
                scalar dx2jk = magSqr(pj - pk);
                
                //midpoints for each edge
                Foam::point midPointij = 0.5*(pi + pj);
                Foam::point midPointik = 0.5*(pi + pk);
                Foam::point midPointjk = 0.5*(pj + pk);
                
                Foam::point centroid = (pi + pk + pj)/3.0;
                
                //dx2Ideal for each edge
                //scalar dx2Idealij = sqr(initialPoints.requiredResolution(midPointij));
                //scalar dx2Idealik = sqr(initialPoints.requiredResolution(midPointik));
                //scalar dx2Idealjk = sqr(initialPoints.requiredResolution(midPointjk));
                scalar dx2Ideal = sqr(initialPoints.requiredResolution(centroid));
                ...
                //if (dx2 > 1.7777777*dx2Ideal)
                //Info << dx2ij << "  " << dx2ik << "  " << dx2jk << "  " << dx2Idealij << "  " << dx2Idealik << "  " << dx2Idealjk << endl; 
		        //if 
		        //(
		        //    (dx2ij > 1.7777777*dx2Idealij && (dx2ik > 1.7777777*dx2Idealik || dx2jk > 1.7777777*dx2Idealjk)) ||
		        //    (dx2ik > 1.7777777*dx2Idealik && dx2jk > 1.7777777*dx2Idealjk)
		        //)
		        if (dx2ij > 4*dx2Ideal || dx2jk > 4*dx2Ideal || dx2ik > 4*dx2Ideal)
                {   //Info << "inserting" << endl;
                    insertNearVerts[nPointsToInsert] = vi;
                    pointsToInsert[nPointsToInsert++] = midPointij;
                    insertNearVerts[nPointsToInsert] = vj;
                    pointsToInsert[nPointsToInsert++] = midPointjk;
                    insertNearVerts[nPointsToInsert] = vk;
                    pointsToInsert[nPointsToInsert++] = midPointik;
                }
            }
        }
