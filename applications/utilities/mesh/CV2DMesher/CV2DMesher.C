...
int main(int argc, char *argv[])
{
    ...
    for (int iter=1; iter<=nIterations; iter++)
    {
        scalar relax = 
        (
           mesh.meshingControls().relaxationFactorEnd*scalar(iter)
         + mesh.meshingControls().relaxationFactorStart*scalar(nIterations-iter)
        )/nIterations;

        Info<< nl
            << "Relaxation iteration " << iter << " relaxation = " << relax << nl
            << "~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
